...
public class KeyByParallelismManualMatcher {
    ...
    public static void newLeftOutput(Tuple2<Long, Tuple2<Long, Long>> item) {
        ...
        synchronized (lock) {
            ArrayList<Tuple2<Long, Tuple2<Long, Long>>> oldItems = leftOutput.getOrDefault(key, new ArrayList<>());
            oldItems.add(item);
    ...
    public static void newRightOutput(Tuple2<Long, Tuple2<Long, Long>> item) {
        ...
        // Add the new item in the map
        synchronized (lock) {
            ArrayList<Tuple2<Long, Tuple2<Long, Long>>> oldItems = rightOutput.getOrDefault(key, new ArrayList<>());
            oldItems.add(item);
    ...
    public static boolean cleanUpPrefixes(Long key) {
        synchronized (lock) {
            ArrayList<Tuple2<Long, Tuple2<Long, Long>>> oldLeft = leftOutput.getOrDefault(key, new ArrayList<>());
            ArrayList<Tuple2<Long, Tuple2<Long, Long>>> oldRight = rightOutput.getOrDefault(key, new ArrayList<>());
