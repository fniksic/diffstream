...
public class WordCountParallel implements FlinkProgram<WordCountItem, String>, Serializable {

    private static final long serialVersionUID = 7748747289925717465L;

    private final int parallelism;

    public WordCountParallel(int parallelism) {
    ...
    public DataStream<String> apply(DataStream<WordCountItem> inputStream) {
        return inputStream.assignTimestampsAndWatermarks(new EndOfFileToWatermarkAssigner()).setParallelism(1)
                .flatMap(new FlatMapFunction<WordCountItem, Word>() {
                    @Override
                    public void flatMap(WordCountItem item, Collector<Word> collector) throws Exception {
                        item.<Void>match(
                                word -> {
                ...
                .map(new RichMapFunction<Word, Tuple2<Word, String>>() {
                    ...
                    @Override
                    public Tuple2<Word, String> map(Word word) throws Exception {
                ...
                .reduce((x, y) -> mergeTopicCounts(x, y))
                .map(WordCountParallel::mostFrequentTopic);
    }
    ...
    private static class EndOfFileToWatermarkAssigner implements AssignerWithPunctuatedWatermarks<WordCountItem> {
        private long eofCount = 0L;

        @Nullable
        @Override
        public Watermark checkAndGetNextWatermark(WordCountItem item, long extractedTimestamp) {
            return item.match(
                    word -> null,
                    endOfFile -> {
                        eofCount++;
        ...
        @Override
        public long extractTimestamp(WordCountItem item, long previousElementTimestamp) {
            return eofCount;
        }
    }
